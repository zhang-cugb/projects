...
class ImplicitTwoPhase_ROM:
    ...
    def solve(self, schedule, basis, mean, max_iter=10, tol=1E-12, ATS = False, 
        linear_solver=None):
        '''Run the solver for the timesteps in the schedule.
        Arguments:
            schedule (object) : schedule
        ...
        while k < timesteps.size and timesteps[:k-1].sum() < maxtime:
            ...
            # Solve with Newton-Raphson Method
            psw_new, iter_number, error, solver_info = NewRaph_ROM(res_Jacbn, psw_new, tol, max_iter, linear_solver, basis, mean)                        
...
def NewRaph_ROM( f, x0,  tol , max_iter, linear_solver, basis, mean):
    '''
    Newton-Raphson method to obtain the root of the function.
    Arguments:
        f (function) :  f(x) = ( residual, jacobian ) 
    ...
    for k in range(0, max_iter):
        ...
        # -------- BEGIN ROM --------------------
        # Project and get reduced residual and jacobian
        J_r = (basis.T.dot(J)).dot(basis)
        fx0_r = basis.T.dot(fx0)

        
        # Solve reduced system
        dx_r = sparse.linalg.spsolve(J_r, fx0_r)
        dx_r = dx_r.reshape([-1,1])
       
        # Project back
        dx = basis.dot(dx_r) #+ mean
